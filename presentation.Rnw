...
print(xtable(res, caption = "Ocena predykcji klasyfikatorów"), include.rownames = FALSE)
@
...
ggplot(pair_dat, aes(x = normal, y =  binary, col = tar, fill = tar)) +
  ...
  scale_fill_hue("Amyloid", l=70, c=150) + 
  scale_x_discrete("Predykcja dla zliczeń n-gramów") +
  scale_y_discrete("Predykcja dla zbinaryzowanych zliczeń n-gramów") + 
  facet_wrap(~ dists)
@
