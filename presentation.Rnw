...
colnames(res) <- c("n", "Długość przerwy", "Binaryzacja", "AUC", "TNR", "TPR")
rws <- seq(1, nrow(res), by = 2)
col <- rep("\\rowcolor[gray]{0.85}", length(rws))
print(xtable(res, caption = "Ocena predykcji klasyfikatorów"), include.rownames = FALSE, booktabs = TRUE,
...
ggplot(aoc, aes(x = dists, y = count, fill = tar)) +
  ...
  scale_y_continuous("Częstość petydów") +
