...
## Uczenie
 ...
 - Z każdego okna wybrano 1-, 2- i 3-gramy (zarówno ciągłe jak i z przerwami). Dla każdego kodowania osobno, n-gramy przefiltrowano używając metody QuiPT i wykorzystano do uczenia lasów losowych [@liaw_classification_2002]. 
 - Procedurę wykonano trzykrotnie na trzech zbiorach uczących: sekwencje o długości 6 aminokwasów, krótsze niż 11 aminokwasów i krótsze niż 15 aminokwasów. 
...
```{r echo=FALSE,message=FALSE,results='asis'}
colnames(res) <- c("Zbiór uczący", "Encoding ID", "AUC", "Specyficzność", "Czułość", "Liczba grup")

res <- res[, c(1, 6, 2, 5, 4)]

datatable(res, filter = "top", options = list(searching = TRUE)) %>% formatRound(4L:7, 4)
```
